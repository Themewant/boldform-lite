...
Improvements:
* Improve: The "Choose a Template" library now also lists the ready-made forms available with an upgrade, so you can see what exists rather than having to guess. They sit below the templates you already have, marked with a padlock; selecting one shows what the form is for. Four categories — Health & Medical, Education & Nonprofit, Payment & Calculation and Multi-Step — were previously empty and never appeared at all.
...
Fixes:
* Fix: A required Terms & Conditions checkbox now shows the red asterisk that marks every other required field, on the form and in the builder. It was already required — the box carried the browser's required flag and an unticked one was refused on submit — but with nothing on screen to say so, it read as optional. This applies to any consent field, not only imported ones.
* Fix: A field's caption is no longer lost when the Contact Form 7 form puts it on its own line with a line break — "Your Name&lt;br&gt;[text* your-name]", one of the commonest layouts. Those fields used to arrive labelled from their tag name instead ("Your name"). Captions written inside a &lt;label&gt; were always picked up correctly and are unchanged.
* Fix: Fields written side by side on the very first line of a Contact Form 7 form now share one row, as they already did everywhere else in the form. They were being stacked one per row, because the first line could not be told apart from "this field could not be located".
* Fix: When a Contact Form 7 form sends its notification to several addresses, the import now says so. Only a single address can be carried over, so the rest quietly fell back to the site administrator with nothing to tell you.
* Fix: The Form Migrator no longer loses an "I accept…" checkbox written as [acceptance* consent]. Contact Form 7 has no starred acceptance tag, so that line is one Contact Form 7 itself cannot read — it shows as plain text on the form — and the import passed over it in silence while an ordinary [acceptance consent] beside it came across fine. It now imports too, in its original position and with its own wording, as an optional field, since the Contact Form 7 form does not require it either; the result panel explains the tag so you can correct it at the source or simply tick Required here. An ordinary acceptance checkbox still imports as required, which is Contact Form 7's rule — it is mandatory unless the tag carries the "optional" keyword. The same form also used to import differently depending on whether Contact Form 7 happened to be active; the two paths now agree. Any form whose layout contained quoted markup or a typographic character — twelve of the ready-made templates did — came in with no fields at all and its settings reset, because the escaping inside the saved layout was stripped while the file was read. The layout is now read exactly as written. A file whose layout genuinely cannot be read is skipped and counted in the notice rather than creating an empty form that looks imported.
