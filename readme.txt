...
* Fix: The Tools -> Entries panel could lose its "Export format" control entirely when an add-on that provides extra formats was installed but not yet entitled to run — the add-on suppressed this plugin's locked-format teaser without supplying its own field. The teaser now has its own switch, so the control is always present.
